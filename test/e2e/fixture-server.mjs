...
function fixtureSession() {
  ...
  return {
    ...
    pendingModel: false,
    badge: badgeForModel(claudeFixture.model),
  };
}

function badgeForModel(modelRef) {
  const lower = String(modelRef || '').toLowerCase();
  if (lower.includes('opus')) return { label: 'OPUS', kind: 'claude' };
  if (lower.includes('sonnet')) return { label: 'SONNET', kind: 'claude' };
  if (lower.includes('haiku')) return { label: 'HAIKU', kind: 'claude' };
...
function handleClaudeFixtureApi(request, response, pathname) {
  ...
  if (decoded === '/api/sessions/web:claude1/commands' && method === 'POST') {
    readJsonBody(request, (body) => {
      ...
      if (body.command === 'pi thinking' && body.args?.level) claudeFixture.thinking = body.args.level;
...
const server = createServer((request, response) => {
  ...
  if (url.searchParams.get('fixture') === 'claude-tmux' && pathname === '/') {
    clearFixtureTimers();
    claudeFixture.nextId = 1;
    claudeFixture.busy = false;
    claudeFixture.model = 'claude-code/haiku';
    claudeFixture.thinking = 'high';
    claudeFixture.events = [];
  }
