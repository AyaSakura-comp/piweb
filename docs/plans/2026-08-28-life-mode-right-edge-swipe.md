...
**Architecture:** Add an explicit `channels.kind` discriminator (`standard | life`) and an idempotent authenticated `POST /api/life-session`. Standard session lists/trash exclude the Life channel. The frontend stores only the presentation mode (`piweb.mode`), uses the existing transcript/composer and integration pipeline for the Life channel, renders simplified Life chrome with a dedicated New-pi-session header pencil and a Search/Media overflow, and recognizes a velocity-aware right-edge entry swipe plus a left-edge back swipe without stealing vertical scrolling. The 44×64 px edge leaf is also an accessible button that auto-settles through the same transition. `pi new` rotates the internal Pi context but preserves the protected Life channel/transcript. A return or rollback with no standard destination must clear all Life selection/stream/search ownership and render an empty standard shell.
...
**Step 1 — RED:** Add source/UI tests and Playwright mobile tests that start within 56 px of the right edge, prove a slow shallow drag cancels, prove a shorter fast flick commits through projected velocity, prove tapping the edge leaf auto-settles the panel, observe the preview continue moving after release, and land in Life mode after one `POST /api/life-session`. Assert a vertical edge gesture does nothing, a rightward left-edge back swipe exits Life safely, automatic travel retains a visible source-page sliver instead of snapping offscreen, ready content crossfades through the underlay, shallow back travel eases home, and the Life header pencil can issue `pi new` to `web:life`.
...
**Step 3 — GREEN:** Add a 44×64 px accessible right-edge leaf button and full-height swipe preview that follows the finger; tapping the leaf auto-settles through the same transition. Axis-lock after 8 px; claim only horizontal motion; commit after 22% or a velocity-projected short flick, require the release direction to remain leftward, then animate the remaining travel with a release-speed-scaled 150–320ms balanced ease. During post-release settlement, make the underlying main/drawer inert, block competing drawer gestures, and expose Cancel; cancellation or any newer navigation invalidates preview ownership even if Life API/history remains delayed. Start entry/exit destination navigation only after the source page is fully covered so a fast response cannot mutate visible content. Once the destination is ready, reset the real page beneath the fixed underlay and crossfade that underlay for 180ms instead of hiding it in the same frame. If exit cancellation or the desktop breakpoint arrives after standard selection began, re-enter Life under a newer navigation generation so the pending standard result cannot commit later. On entry, remember `piweb.mode=life`, open the singleton Life channel, and render `Sessions / Life / DEFAULT` chrome while hiding drawer, rename/delete/clear, model, thinking, status, and usage controls. Keep a dedicated New pi session pencil immediately before ⋯ and a Life-safe overflow containing Search and Media, plus the composer and attachment integrations. Add a `Sessions` action plus a 36px left-edge rightward back swipe; both settle the Life page right over a Sessions underlay before restoring the remembered standard session, while shallow back drags ease home. If no standard target exists on return or failed entry, invalidate selection/search generations, close SSE, clear Life transcript/partial/busy state and active JID, and render the empty standard shell.

**Step 4 — Verify GREEN:** Assert default badge, hidden management rows, a working New-pi-session header pencil plus Search/Media overflow actions, slow-drag cancellation, short-flick inertia, balanced entry travel, underlay-to-content crossfade, left-edge swipe-back cancellation/commit/ease-home, smooth Sessions-button exit, persistent reload entry, back navigation with and without standard sessions, failed-history rollback with no standard sessions, no horizontal overflow, visual viewport containment, pointer reachability, and message POST isolation from the Life JID after empty fallback.
