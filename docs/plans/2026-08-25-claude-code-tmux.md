...
### Resumption status (2026-09-22)

- Core adapter, routing, reset and regression fixes verified: 58 focused unit tests pass.
- Real opt-in Haiku smoke passes: tools/uploads, reuse/resume, Stop, post-Stop turn, fresh UUID.
- Follow-up full suite: 550 unit tests passed, 1 opt-in skipped; 119 mobile E2E
  passed, 12 opt-in skipped. Repository lint/build/typecheck pass.
- Real opt-in browser E2E passes: API → SQLite → control/message loops → Claude
  tmux → SSE; Chinese file tool output, Stop, continuation, pane resume and reload.
- Dark/light continuous UI recording passes, including the unsupported-effort
  note regression. Continuous real-Haiku video also recorded; rejected initial
  sleep-based workload is retained separately, not presented as passing evidence.
- Setup/security/lifecycle docs: `docs/claude-tmux-bridge.md`.
- Latest evidence: `artifacts/claude-bridge-video-20260922/report.md`.
- Independent review, direct video playback review and deployed HTTPS smoke remain
  unperformed. Timeline samples at 2fps were inspected, not every video frame.
  No commit, push, production enablement or restart performed; preserve unrelated edits.
